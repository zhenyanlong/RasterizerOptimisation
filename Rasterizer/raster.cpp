...
void RenderScene(Renderer& renderer, std::vector<Mesh*>& scene, matrix& camera, Light& L/*, threadpool*/) {
    // Call the desired scene function to render
    //sceneTest();
    // achieve tbr rendering
	// clear triangle list
	renderer.tileManager.clear();

	// traverse the scene and render each mesh
    for (auto& m : scene)
    {
		// Combine perspective, camera, and world transformations for the mesh
...
#pragma unroll(3)
			for (unsigned int i = 0; i < 3; i++) {
				t[i].p = p * m->vertices[ind.v[i]].p; // Apply transformations
				t[i].p.divideW(); // Perspective division to normalize coordinates
			...
			triangle tri(t[0], t[1], t[2]);
			//tri.draw(renderer, L, mesh->ka, mesh->kd);
			//triangleList.push_back(tri); // Store the triangle for later use in tile-based rendering
			vec2D minV, maxV;
			tri.getBoundsWindow(renderer.canvas, minV, maxV);
            // put all triangle to a vector and allocate to different tiles for parallel rendering
			renderer.tileManager.addTriangleToTiles(minV, maxV, tri); // Add triangle to tile manager

		}
    }
	// draw with threadpool
}
